...
const BE_URL = `${BASE_URL}/api/v1/inventory`;
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (contentType && contentType.includes("text/html")) {
      console.error("DEBUG BE: Rute GET inventory tidak ditemukan di Backend (404)");
    ...
    console.error("ERROR PROXY GET INVENTORY:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const contentType = res.headers.get("content-type");
    if (contentType && contentType.includes("text/html")) {
      console.error("DEBUG BE: Rute POST inventory tidak ditemukan di Backend (404)");
      return NextResponse.json({ success: false, message: "Backend Endpoint salah" }, { status: 404 });
    }
    ...
    console.error("ERROR PROXY POST INVENTORY:", error);
    return NextResponse.json({ success: false, message: "Internal Server Error FE" }, { status: 500 });
  }
}