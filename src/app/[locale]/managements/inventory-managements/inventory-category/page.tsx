...
import {
  ...
  Eye, Edit2, Trash2,
  X, AlertTriangle,
  Tag, Hash, ChevronDown,
  ArrowUp, ArrowDown, ArrowUpDown, Filter,
  List, BarChart2,
  LayoutGrid, Package, PackageCheck, Wrench, PackageX,
  TrendingUp, Layers
} from "lucide-react";
...
export default function InventoryCategoryPage() {
  const t = useTranslations("InventoryCategoryPage");
  ...
  const [newItem, setNewItem] = useState({ ...EMPTY_CATEGORY });
  ...
  const fetchCategories = useCallback(async () => {
    setIsLoading(true);
    try {
      const url = new URL(`/api/v1/inventory/categories`, window.location.origin);
  ...
  const handleAdd = async () => {
    await fetch(`/api/v1/inventory/categories`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${getToken()}` },
      body: JSON.stringify(newItem)
    });
    setIsAddOpen(false);
    setNewItem({ ...EMPTY_CATEGORY });
    fetchCategories();
  };

  const handleEdit = async () => {
    if (!editItem) return;
    await fetch(`/api/v1/inventory/categories/${editItem.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${getToken()}` },
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    await fetch(`/api/v1/inventory/categories/${deleteId}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${getToken()}` }
    });
    setDeleteId(null);
  ...
  const analysisData = useMemo(() => {
    ...
    const empty = categories.filter(c => (c.itemCount ?? 0) === 0).length;
    const totalItemsInCategories = categories.reduce((acc, c) => acc + (c.itemCount ?? 0), 0);
    const topCategories = [...categories]
      .sort((a, b) => (b.itemCount ?? 0) - (a.itemCount ?? 0))
      .slice(0, 5);
    return { withItems, empty, totalItemsInCategories, topCategories };
  }, [categories]);
  ...
  const tableHeaders = [
    ...
    { key: "description", label: t("table.description"), sortable: false },
  ...
  const setCurrentForm = (val: Partial<Category>) => {
    ...
    <div className="p-4 md:p-6 max-w-400 mx-auto flex flex-col min-h-screen text-foreground">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
        <div className="flex gap-1 p-1 bg-bg-alt border border-border rounded-lg w-fit overflow-x-auto scrollbar-hide">
          {[
            { key: "list", label: "Category List", icon: List },
      ...
      {activeTab === "analysis" && (
        <div className="space-y-6 animate-in fade-in duration-300">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: "Total Categories", val: totalItems, icon: Layers, color: "text-blue-500", bg: "bg-blue-500/10" },
              ...
              { label: "Total Items", val: analysisData.totalItemsInCategories, icon: Hash, color: "text-purple-500", bg: "bg-purple-500/10" },
            ].map((stat) => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-bg-alt border border-border rounded-lg shadow-sm">
              <div className="p-5 border-b border-border/60 font-bold text-primary flex items-center gap-2 justify-center">
                <TrendingUp size={18} /> Top Categories by Items
              </div>
              <div className="p-5 space-y-4">
                {analysisData.topCategories.length === 0 ? (
                  ...
                  const pct = analysisData.totalItemsInCategories > 0
                    ? ((cat.itemCount ?? 0) / analysisData.totalItemsInCategories) * 100
                    : 0;
                  return (
            ...
            <div className="bg-bg-alt border border-border rounded-lg shadow-sm">
              <div className="p-5 border-b border-border/60 font-bold text-primary flex items-center gap-2 justify-center">
                <Tag size={18} /> All Categories Ranking
              </div>
              <div className="divide-y divide-border/40">
                {categories.length === 0 ? (
                  ...
                  .sort((a, b) => (b.itemCount ?? 0) - (a.itemCount ?? 0))
                  .map((cat, i) => (
                    <div key={cat.id} className="flex items-center justify-between p-4 px-5 text-left">
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-sm font-bold text-foreground truncate max-w-48">{cat.name}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-xs font-black text-foreground">{cat.itemCount ?? 0}</p>
          ...
          {showFilters && (
            <div className="p-4 border-b border-border/60 bg-background/30 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-in fade-in slide-in-from-top-2">
              <input
                ...
                className="p-2 text-sm bg-background border border-border rounded-lg focus:border-primary outline-none text-foreground col-span-full sm:col-span-2 lg:col-span-2"
              />
            ...
            <table className="w-full text-center border-collapse min-w-200">
              ...
              <tbody className="divide-y divide-border/40 text-center">
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={5} className="py-20 text-center text-text-muted text-sm">
                      No categories found.
                    </td>
                  </tr>
                  ...
                  <tr key={item.id} className="hover:bg-background/40 transition-colors text-sm">
                    ...
                    <td className="py-4 px-4 text-text-muted text-xs max-w-xs truncate">
          ...
          >
            <div className="flex justify-between items-center p-5 border-b border-border/50">
              <h3 className="font-bold text-lg text-foreground flex items-center gap-2">
                <Tag className="text-primary" size={20} /> Category Detail
              </h3>
            ...
            <div className="p-6 space-y-4">
              {[
                { icon: Tag, label: "Category Name", value: viewItem.name },
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-left">
                  ...
                  <p className="text-sm text-foreground mt-0.5">{viewItem.description || <span className="italic opacity-40">No description</span>}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        >
          ...
          >
            ...
            <div className="p-6 space-y-3">
              <div>
                <label className="block text-xs font-bold text-text-muted uppercase tracking-widest mb-1.5">Category Name</label>
                <input
                  placeholder="e.g. Electronics"
                  value={currentForm.name}
                  onChange={(e) => setCurrentForm({ name: e.target.value })}
